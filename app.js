...
function renderEducation(){
  ...
  el.innerHTML = EDUCATION.map(function(e){
    return '<div class="edu-item"><div class="edu-top"><span class="tl-co-logo edu-logo"><img src="https://www.google.com/s2/favicons?sz=128&domain=' + e.domain + '" onerror="lf(this,\'' + e.mono + '\',\'' + e.sw + '\')"></span>'
      ...
      + '<p class="ml">Relevant Modules</p><div class="mod-grid">'
      + e.modules.map(function(m, i){ return '<span class="mt"><i class="mi sw' + (i % 5) + '">' + m[0] + '</i>' + m[1] + '</span>'; }).join('')
      + '</div></div>';
  }).join('');
}

function renderSkills(){
  document.getElementById('sk-biz').innerHTML = SKILLS_BUSINESS.map(function(s){
    return '<span class="hi"><i class="fa-solid ' + s[0] + '"></i>' + s[1] + '</span>';
  }).join('');
  document.getElementById('sk-tech').innerHTML = SKILLS_TECH.map(function(s){
...
function renderHobbies(){
  ...
  el.innerHTML = HOBBIES.map(function(h){
    return '<div class="cert-card hobby-card"><i class="fa-solid ' + h.icon + ' hobby-ic"></i><p>' + h.title + '</p><span class="cert-year">' + h.desc + '</span></div>';
  }).join('');
}
