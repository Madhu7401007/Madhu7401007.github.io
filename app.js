...
function renderEducation(){
  ...
  el.innerHTML = EDUCATION.map(function(e){
    return '<div class="edu-item"><div class="edu-top"><span class="tl-co-logo edu-logo"><img src="https://www.google.com/s2/favicons?sz=128&domain=' + e.domain + '" onerror="lf(this,\'' + e.mono + '\',\'' + e.sw + '\')"></span>'
      ...
      + '<div class="edu-badges"><span class="edu-badge">' + e.honours + '</span>' + (e.extra ? '<span class="edu-badge edu-badge-alt">' + e.extra + '</span>' : '') + '</div>'
      + (e.highlight ? '<p class="edu-highlight"><i class="fa-solid fa-trophy"></i> ' + e.highlight + '</p>' : '')
...
function renderQuickFacts(){
  ...
  el.innerHTML = SKILL_GROUPS.map(function(g){
    return '<p class="qf-label">' + g.label + '</p><div class="pills qf-pills">'
      + g.items.slice(0,3).map(function(s){ return '<span>' + s.label + '</span>'; }).join('')
      + '</div>';
  }).join('');
}
